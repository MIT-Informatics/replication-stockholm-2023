...
# reproduce authors model using original code for OLS with 'robust' errors & base R OLS

lmspec <- "dv_pop_01*pct_noneu_06"
results_computational.lmr <-
  estimatr::lm_robust(formula(paste0("d_rr_06 ~", lmspec)), 
                      data = austria_authors.df)

results_ols.lm <-
  stats::lm(formula(paste0("d_rr_06 ~", lmspec)), data = austria_authors.df)
```

```{r tidy-results}

tidy_results.df  <- 
  map(list(comp=results_computational.lmr, ols=results_ols.lm), \(x) tidy(x)) %>%
  list_rbind(names_to="repl_id") %>%
  bind_rows(tidy_original.df)

glance_summary_results.df <- 
   map(list(comp=results_computational.lmr, ols=results_ols.lm), \(x) glance(summary(x))) %>%
  list_rbind(names_to="repl_id") 

print(tidy_results.df)
print(glance_summary_results.df)
