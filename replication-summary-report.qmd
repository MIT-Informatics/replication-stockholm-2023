...
austria_authors.df %>%
  ...
  geom_point() +
  labs(
    x = "% non-EU residents in municipality",
...
Visual examination of the data indicates that (1) there is a _very_ modest
association between the percentage of third-country nationals and both the level
of support and the _change_ in the level of support for far-right parties in the
...
these two factors---i.e., that support is driven by competition between
Austrian and third-country nationals for housing, which is most accute when
public housing rates and the proportion of the population that is third-country
...
cuts <- with(austria_authors.df, quantile(dv_pop_01, probs = seq(0, 1, 1/3),
                                          na.rm = TRUE))
austria_authors.df %>%
...
vienna_authors.df %>%
  ...
  geom_point() +
  labs(
    x = "% of residents in public housing",
    y = "Change in far-right vote share"
  ) +
...
cuts <- with(vienna_authors.df, quantile(pctpublic_w_zsp, probs = c(0, 1/2, 3/4, 1),
                                         na.rm = TRUE))
vienna_authors.df %>%
