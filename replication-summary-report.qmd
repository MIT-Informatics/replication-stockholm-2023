...
# **A structured comment on Cavaillé and Ferwerda (2022)**
...
We find that the research is computationally replicable but the evidence for the main causal claims are potentially overstated as it sensitive to a small set of observations, does not explain most of the variation in outcomes, and does not out-compete simpler causal explanations.
...
Cavaillé and Ferwerda [@cavaillé2023] tested the effect of granting immigrants access to the welfare state in Austria during 2006 on the support for far-right parties. We focus on their two main claims, as described in the abstract (pg 19):
...
Subsequently, we were asked to review a response from the author to our report. We allocated approximately two days to this activity – which resulted in some additions to the text of this report to further clarify the intended interpretation of our comments.
...
repro_m1.df %>% group_by(repl_id) %>% 
  fmtTB(caption="coefficients")

repro_m1_summary.df %>% group_by(repl_id) %>% 
    fmtTB(caption="summaries")
...
repro_m2_summary.df %>% group_by(repl_id) %>%
      fmtTB(caption="summaries")
```
...
alt_est_m1.df  %>% group_by(repl_id) %>% 
        fmtTB(caption="coefficients")
alt_est_m1_summary.df %>% group_by(repl_id) %>% 
        fmtTB(caption="summaries")
...
alt_est_m2_summary.df %>% group_by(repl_id) %>%
        fmtTB(caption="summaries")
...
alt_var_m1_summary.df %>% group_by(repl_id) %>% 
        fmtTB(caption="summaries")
...
alt_var_m2_summary.df %>% group_by(repl_id) %>% 
        fmtTB(caption="summaries")
...
Observe as with the primary finding that none of the alternative models fit the data well, and that models without interaction effects provide only slightly lower explanatory power. Further, when the original model applied to the country as a whole (as above) for the main finding is fit to this subset, the interaction term is not significant.

```{r AIC-m2}
m2.formula<- formula("dv ~  (pctrental + pctpublic_w_zsp)")
results_m2_computational.lm <-
  lm(m2.formula,   data = vienna_authors.df)

results_m2_interaction.lm <-
lm(formula("dv ~ pctrental*pctpublic_w_zsp"),
                      data = vienna_authors.df)

results_m2_m1model.lm <-
 lm(formula("dv ~ pctpublic_w_zsp*pctforeign"),
                      data = vienna_authors.df)

results_m2_rental.lm <-
  lm(formula("dv ~ pctrental"),
                      data = vienna_authors.df)

results_m2_housing.lm <-
  lm(formula("dv ~ pctpublic_w_zsp"),
                      data = vienna_authors.df)

ml.ls <- list(author_model2 = results_m2_computational.lm,
              interaction = results_m2_interaction.lm,
              m1spec = results_m2_m1model.lm,
              housing_only = results_m2_housing.lm,
              rental_only = results_m2_rental.lm
              )
...
AICcmodavg::evidence(bictab.tb, model.high="author_model2", model.low="interaction")
...
repro_m1_reweight_summary.df %>% group_by(repl_id) %>%
      fmtTB(caption="summaries")
...
repro_m2_reweight_summary.df %>% group_by(repl_id) %>%  fmtTB(caption="summaries")
...
tidy_results(list(results_m2_pctforeign_reweight)) %>% fmtTB(caption="coefficients")

tidy_summary(list(results_m2_pctforeign_reweight)) %>% fmtTB(caption="summaries")
...
tidy_summary(ml.ls)%>%
  fmtTB(caption="summary")

rm(ols1,ols2,ols3,ols4,ml.ls,
   m1_ols4.felm,
   m1_ols4.felm,
   m1_ols4.felm,
   m1_ols4.felm)
...
rm(ml.ls, rols1, rols2, rols3, rols4, m1rols1.felm, m1rols2.felm,m1rols3.felm,m1rols4.felm)
```
